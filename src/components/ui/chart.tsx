  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        data-chart={chartId}
        className={cn(
          "flex aspect-auto min-h-[220px] w-full justify-center overflow-hidden text-xs sm:min-h-[256px] [&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/50 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-border [&_.recharts-radial-bar-background-sector]:fill-muted [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-muted [&_.recharts-reference-line_[stroke='#ccc']]:stroke-border",
          className,
        )}
        {...props}
      >
...
const ChartTooltip: any = RechartsPrimitive.Tooltip;
...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, any>(
  ({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey }, ref) => {
    ...
    return (
      <div ref={ref} className={cn("grid max-w-[min(280px,90vw)] min-w-32 items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className)}>
        ...
        <div className="grid gap-1.5">
          {payload.map((item: any, index: number) => {
            ...
            return (
              <div key={item.dataKey as string} className={cn("[&>svg]:text-muted-foreground flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5", indicator === "dot" && "items-center")}>
...
const ChartLegend: any = RechartsPrimitive.Legend;
      ...
      {payload.map((item: any) => {
        ...
        return (
          <div key={item.value} className="flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground">
            {itemConfig?.icon && !hideIcon ? <itemConfig.icon /> : !hideIcon && <div className="h-2 w-2 shrink-0 rounded-[2px]" style={{ backgroundColor: item.color }} />}
            {itemConfig?.label}
          </div>
        );
      })}
