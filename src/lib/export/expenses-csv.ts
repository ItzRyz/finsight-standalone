...
export function expensesToCsv(expenses: ExpenseCsvRow[]) {
  const income = expenses.filter((x) => x.type === "INCOME").reduce((s, x) => s + Number(x.amount), 0);
  const expense = expenses.filter((x) => x.type === "EXPENSE").reduce((s, x) => s + Number(x.amount), 0);
  const headers = ["Date", "Title", "Type", "Category", "Amount", "Merchant", "Location", "Description", "Receipt"];
